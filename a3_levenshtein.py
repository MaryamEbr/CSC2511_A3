...
def cleanText(text):
    # remove the label part from the beginning
    text = text[text.find("  "):]
    # remove tag like <LG>
    notag = re.compile('<.*?>')
    text = re.sub(notag, '', text)
    # remove things that come in between [], like [laughter]
    nobracket = re.compile('\[.*?\]')
    text = re.sub(nobracket, '', text)
    # remove punctuations
